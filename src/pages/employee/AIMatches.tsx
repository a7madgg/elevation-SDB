...
import { catLabel, cityLabel, useT } from '@/i18n'
...
export default function AIMatches() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 pt-4 pb-10 sm:px-6 lg:px-8 lg:pt-2">
      <SectionHeader
        eyebrow={t('employee.matchesEyebrow')}
        title={t('employee.matchesTitle')}
        description={t('employee.matchesDesc')}
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-6 flex-1 min-w-0">
                    <div className="min-w-0">
                      ...
                      <Badge tone="orange" className="mt-1.5">{t('employee.needs', { cat: catLabel(t, m.seekerNeeds) })}</Badge>
                    </div>
                    <ArrowDown size={16} className="text-[#c9d3d8] rotate-[-90deg] rtl:rotate-90 shrink-0" />
                    <div className="min-w-0">
                      <p className="text-[14.5px] font-bold text-sdb-deep truncate">{m.providerName}</p>
                      <p className="flex items-center gap-1 text-[12px] text-[#95a2a9]"><MapPin size={11} /> {cityLabel(t, m.city)}</p>
                      <Badge tone="green" className="mt-1.5">{t('employee.provides', { cat: catLabel(t, m.providerProvides) })}</Badge>
                    </div>
                  </div>
                  <div className="text-end shrink-0">
                    <p className="text-[22px] font-extrabold" style={{ color }}>{m.matchScore}%</p>
                    <p className="text-[10.5px] text-[#95a2a9]">{t('common.match')}</p>
                  </div>
                </div>
                <div className="mt-3">
