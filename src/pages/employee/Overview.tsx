        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <p className="flex items-center gap-1.5 text-[13px] font-bold text-sdb-deep">
              <Sparkles size={14} className="text-sdb-cyan" /> Ecosystem Potential
            </p>
            <DemoDataBadge label="Illustrative simulation" />
          </div>
          <p className="mt-1 text-[12.5px] text-[#6b7a83]">What AI estimates is possible if every unmet need in the ecosystem were matched.</p>
          <div className="mt-4 grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatTile label="Potential connections" value={ecosystemPotential.potentialConnections} format={formatCompact} icon={<GitMerge size={16} />} accent="#12B1C6" delay={0} />
            <StatTile label="Unmet needs" value={ecosystemPotential.unmetNeeds} format={formatCompact} icon={<Heart size={16} />} accent="#F0693E" delay={0.05} />
            <StatTile label="Potential collaborations" value={ecosystemPotential.potentialCollaborations} format={formatCompact} icon={<Handshake size={16} />} accent="#34B889" delay={0.1} />
            <StatTile label="Businesses needing support" value={ecosystemPotential.businessesNeedingSupport} format={formatCompact} icon={<ShieldAlert size={16} />} accent="#70154C" delay={0.15} />
          </div>
        </Card>
      </motion.div>

      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <QuickLink title="Business Resilience" desc={`${formatCompact(earlyWarningCount)} businesses flagged by AI early-warning`} onClick={() => navigate('/employee/resilience')} color="#F0693E" />
